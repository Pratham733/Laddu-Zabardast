...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    {...props}
  >
    <span className="flex items-center flex-1 min-w-0 truncate select-value">
      {React.Children.map(children, child => {
        if (
          React.isValidElement(child) &&
          (child.type === SelectValue || 'className' in (child.props || {}))
        ) {
          return React.cloneElement(child, {
            ...(child.props.className ? { className: cn("truncate", child.props.className) } : {}),
          });
        }
        return child;
      })}
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="h-4 w-4 opacity-50 select-arrow" />
